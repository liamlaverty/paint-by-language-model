...
sys.path.insert(0, str(Path(__file__).parent.parent / "src" / "paint_by_language_model"))
...
def main():
    ...
    try:
        ...
        print(f"  Position: ({response['stroke']['start_x']}, {response['stroke']['start_y']}) "
        ...
        iteration_datetime = datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]  # Include milliseconds
