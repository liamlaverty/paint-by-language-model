...
class TestConfigSettings(unittest.TestCase):
    ...
    def test_anthropic_vlm_model(self) -> None:
        """ANTHROPIC_VLM_MODEL defaults to claude-haiku-4-5."""
        self.assertEqual(config.ANTHROPIC_VLM_MODEL, "claude-haiku-4-5")

    def test_anthropic_evaluation_vlm_model(self) -> None:
        """ANTHROPIC_EVALUATION_VLM_MODEL defaults to claude-haiku-4-5."""
        self.assertEqual(config.ANTHROPIC_EVALUATION_VLM_MODEL, "claude-haiku-4-5")

    def test_anthropic_planner_model(self) -> None:
        """ANTHROPIC_PLANNER_MODEL defaults to claude-haiku-4-5."""
        self.assertEqual(config.ANTHROPIC_PLANNER_MODEL, "claude-haiku-4-5")
    ...
    def test_api_base_url_resolves_for_anthropic_provider(self) -> None:
        ...
        try:
            ...
            self.assertEqual(config.VLM_MODEL, "claude-haiku-4-5")
            self.assertEqual(config.EVALUATION_VLM_MODEL, "claude-haiku-4-5")
            self.assertEqual(config.PLANNER_MODEL, "claude-haiku-4-5")
        finally:
